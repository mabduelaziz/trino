...
public abstract class AbstractTestOssFileSystem
        extends AbstractTestTrinoFileSystem
{
    ...
    private OssFileSystemFactory ossFileSystemFactory;
            ...
            throws IOException
    {
        ...
        OssFileSystemConfig config = new OssFileSystemConfig().setJsonKey(new String(jsonKeyBytes, UTF_8));
        OssStorageFactory storageFactory = new OssStorageFactory(config);
        this.ossFileSystemFactory = new OssFileSystemFactory(config, storageFactory);
        ...
        this.fileSystem = ossFileSystemFactory.create(ConnectorIdentity.ofUser("test"));
        cleanupFiles();
    }
    ...
    void tearDown()
    {
        try {
            ...
            rootLocation = null;
            try {
                ossFileSystemFactory.stop();
            }
            finally {
                ossFileSystemFactory = null;
            }
        }
    }
    ...
    {
        String bucket = new OssLocation(rootLocation).bucket();
    ...
    void testExistingFileWithTrailingSlash()
            throws IOException
    {
        BlobId blobId = BlobId.of(new OssLocation(rootLocation).bucket(), "data/file/");
