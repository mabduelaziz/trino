...
{
    public static final String OSS_OAUTH_KEY = "oss.oauth";
    ...
    private final boolean useOssAccessToken;
    ...
    @Inject
    public OssStorageFactory(OssFileSystemConfig config)
            throws IOException
    {
        config.validate();
        projectId = config.getProjectId();
        endpoint = config.getEndpoint();
        useOssAccessToken = config.isUseOssAccessToken();
    ...
    public Storage create(ConnectorIdentity identity)
    {
        try {
            ...
            if (useOssAccessToken) {
                String accessToken = nullToEmpty(identity.getExtraCredentials().get(OSS_OAUTH_KEY));
