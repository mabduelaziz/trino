...
record OssLocation(Location location)
{
    OssLocation
    {
        ...
        checkArgument(location.scheme().get().equals("gs"), "Wrong scheme for OSS location: %s", location);
        ...
        checkArgument(!location.path().contains("#"), "OSS generation numbers are not supported: %s", location);
