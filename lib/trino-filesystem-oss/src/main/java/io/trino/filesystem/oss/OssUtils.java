...
public class OssUtils
{
    ...
    public static IOException handleOssException(RuntimeException exception, String action, OssLocation location)
            throws IOException
    {
        if (exception instanceof BaseServiceException) {
            throw new TrinoFileSystemException("Oss service error %s: %s".formatted(action, location), exception);
        }
        throw new IOException("Error %s: %s".formatted(action, location), exception);
    }

    public static IOException handleOssException(RuntimeException exception, String action, Collection<Location> locations)
            throws IOException
    {
        if (exception instanceof BaseServiceException) {
            throw new TrinoFileSystemException("OSS service error %s: %s".formatted(action, locations), exception);
        }
        throw new IOException("Error %s: %s".formatted(action, locations), exception);
    }

    public static ReadChannel getReadChannel(Blob blob, OssLocation location, long position, int readBlockSize, OptionalLong limit, Optional<EncryptionKey> key)
            throws IOException
    {
    ...
    public static Optional<Blob> getBlob(Storage storage, OssLocation location, Storage.BlobGetOption... blobGetOptions)
    {
    ...
    public static Blob getBlobOrThrow(Storage storage, OssLocation location, Storage.BlobGetOption... blobGetOptions)
            throws IOException
    {
